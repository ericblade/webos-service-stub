describe('webos-service-stub subscribeMethod', () => {
    let stub;
    ...
    it('triggers the request event', (done) => {
        ...
        stub.subscribeMethod('/test', { subscribe: true });
    });
    ...
    it('triggers the request event with correct message.payload', (done) => {
        ...
        stub.subscribeMethod('/test', { test: true, subscribe: true });
    });
    it('emitter response event is triggered by message.respond', (done) => {
        stub.register('/test', (message) => message.respond({}));
        const e = stub.subscribeMethod('/test', { subscribe: true });
        e.on('response', () => done());
    });
    it('default behavior for unknown method calls returns { returnValue: true }', (done) => {
        delete process.env.FAIL_UNKNOWN_LUNA_SERVICES;
        const e = stub.subscribeMethod('/badMethod', { subscribe: true });
        e.on('response', ({ payload }) => {
            expect(payload).to.deep.equal({ returnValue: true, subscribed: true });
            done();
        });
    });
    it('setting environment FAIL_UNKNOWN_LUNA_SERVICES causes unknown method calls to error and cancel', (done) => {
        process.env.FAIL_UNKNOWN_LUNA_SERVICES = '1';
        const e = stub.subscribeMethod('/badMethod', { subscribe: true });
        e.on('response', ({ payload }) => {
            assert.fail(`response not expected ${JSON.stringify(payload)}`)
        });
        e.on('cancel', ({ payload }) => {
            expect(payload).to.deep.equal({
                returnValue: false,
                errorCode: -1,
                errorText: 'Unknown method "/badMethod" for category "/"',
                subscribed: false,
            });
            delete process.env.FAIL_UNKNOWN_LUNA_SERVICES;
            done();
    ...
    it('multiple calls to message.respond are received at the emitter', (done) => {
        ...
        const e = stub.subscribeMethod('/ping', { subscribe: true });
        e.on('response', ({ payload }) => {
