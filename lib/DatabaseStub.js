...
class DatabaseStub {
    ...
    find({ payload: { query }, respond } = {}) {
        // TODO: watch behavior. I thought watch returned any results first, then
        // would later message with a "fired" if the results were changed by a
        // later change, but the documentation seems to indicate that the only
        // response given with a "watch": true is the "fired" response.  If that's
        // the case, then we can simply route find watches to /watch, otherwise
        // we need to run this query first, then add to the watches list.
    ...
    // TODO: /find and /search should also support watches.
    watch(message) {
        const { payload: { query }, respond } = message;
        if (query) {
            // TODO: probably need to test that the query is even valid first
            this.watches.push({ query, message });
            // apparently, /watch doesn't return until the watch actually fires, so this isn't a
            // subscription/watch style method
        } else {
            respond({ todo: 'check out how db8 responds to an invalid db/watch' });
        }
    }
    ...
    checkWatches() {
        this.watches = this.watches.filter((watch) => {
            ...
            const { query: { from, where }, message } = watch;
            const res = this.dataStore.query(from, where);
            if (res.length) {
                message.respond({ returnValue: true, fired: true });
    ...
    constructor(serviceName) {
        ...
        this.dataStore.on('del', this.checkWatches);
        this.dataStore.on('put', this.checkWatches);
        this.dataStore.on('merge', this.checkWatches);
        this.dataStore.on('load', this.checkWatches);
        this.watches = [];
